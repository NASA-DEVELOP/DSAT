...
# 
# checks which CHIRPS data is missing, fetches those missing, and
# clips it to the area of interest.

downClipCHIRPS <- function(progress) { 
  ...
  #directories
  DIR_RAW     <- "data/downloadCHIRPS/raw/"
  DIR_CLIPPED <- "data/downloadCHIRPS/clipped/"
  DIR_PRECIP  <- "data/process/precip/"
  ...
  n <- 10
  progress$inc(1/n, detail = paste("Checking for new CHIRPS Data"))
  enddate=Sys.Date()
  ...
  links <- links[!(names%in%list.files(DIR_CLIPPED))]
  #amend names list based on what actually needs to be downloaded
  # names <- names[!(names%in%list.files(DIR_CLIPPED))]
 ...
 files <- list.files(DIR_RAW, full.names = TRUE)
  #List contents of unprocessed CHIRPS folder.
  ...
  CHIRPSProcess <- function(CHIRPSPrecip){
    ##Define boundaries.
    xmin = -180
    xmax = 180
    ymin = -50
  ...
  #Use CHIRPSProcess function to read in hdf5 files, convert them to rasters, define their projections, and crop to a smaller extent.
  outfiles <- list.files(DIR_RAW, full.names = TRUE)
  outlength <- length(outfiles)
 
  #cropped <- lapply(outfiles, CHIRPSProcess)
  names <- names[1:length(outfiles)]
  for (i in (seq(1, outlength))) {
    xmin = -180
    xmax = 180
    ymin = -50
    ymax = 50
    myrast <- raster(outfiles[i])
    print (outfiles[i])
    extent(myrast) <- c(xmin,xmax,ymin,ymax)
    projection(myrast) <- CRS("+proj=longlat +datum=WGS84")

    e <- extent(c(-113.149999004,-104.349998873,33.2999997512,38.499149))
    cropped_rast <- crop(myrast,e)

    writeRaster(cropped_rast, filename=paste0(DIR_CLIPPED, names[i]), format="GTiff", overwrite = TRUE)
  }
  ...
  #Copy from the clipped folder in the downloadCHIRPS folder to the precip folder in the process folder
  outfiles <- list.files(DIR_CLIPPED, full.names = TRUE)
  file.copy(from=outfiles, to=DIR_PRECIP, 
